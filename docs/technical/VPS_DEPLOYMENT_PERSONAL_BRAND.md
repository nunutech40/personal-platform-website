...
Node/npm   installed
Elixir     not installed yet
Mix        not installed yet
...
- Confirm `nunugraha.web.id` DNS is active in Cloudflare.
- Install Elixir/Mix on the VPS. Do not use Ubuntu apt `elixir` package because it is `1.14`; this app requires `~> 1.15`.
- Fill the production `.env` with real values.
- Restore local data/uploads so no CMS content is left behind, then run migrations.
...
Elixir/Mix are not installed yet on the VPS. Install Erlang + Elixir before deploying. The project requires Elixir `~> 1.15`; Elixir `1.16+` is recommended.
...
# Install Erlang/Elixir using asdf, mise, or official Erlang/Elixir packages.
# Avoid the Ubuntu apt elixir package unless it provides >= 1.15.
...
- Replace `ADMIN_USERNAME` and `ADMIN_PASSWORD` in the VPS env file before exposing `/nunu-ops-7f3c`.
...
- Production database/user are not created yet.
- Domain DNS is active in Cloudflare.
- Data migration decision: restore local CMS data/uploads.
